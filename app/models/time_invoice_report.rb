...
#Start Date=====================================================================
    ...
    #    unless @report_options[:start_date_to].blank?
    #     @time_invoices =@time_invoices.where('start_date <= ?', @report_options[:start_date_to])
    #    end
#===============================================================================

#End Date=======================================================================    
     
    #    if   Date.parse(end_date_from).strftime('%Y-%m-%d %H:%M:%S') > Date.parse(end_date_to).strftime('%Y-%m-%d %H:%M:%S')
...
#Submitted By User==============================================================  
    unless @report_options[:submitted_by_user].blank?
      @time_invoice_details = @time_invoice_details.where("#{TimeInvoice.table_name}.submitted_by_id" =>
          @report_options[:submitted_by_user])
      puts "#{'*'*80}\nTime Invoice Submitted by User #{@time_invoice_details.count(:all)}\n#{'*'*80}"
    end
...
#Invoiced Time==================================================================
        ...
        unless @report_options[:invoiced_time_compared_hours].blank?
          ...
          puts "#{'%'*80}\nInside Time Invoice Report #{@report_options[:invoiced_time_compared_hours]}\n#{'*'*80}"
          ...
          if invoiced_operator_value =='<'
            ...
            sum_invoiced_time_users = sum_invoiced_time_users.group(:user_id).
              having('SUM(invoiced_hours) < ?', @report_options[:invoiced_time_compared_hours])
...
#===============================================================================
    ...
    # @invoice_data=@time_invoices
    #  puts "#{'*'*80}\nThis object #{@reports_reference.all}\n#{'*'*80}"
     puts "#{'F'*80}\nTime invoice End Time #{@time_invoice_details.count(:all)}\n#{'*'*80}"
