module TimeInvoiceReportsHelper
  ...
  def select_project_options(selected_projects_from_view)
    available_projects = Project.all.sort_by{|e| e[:firstname]}
    selected_projects = selected_projects_from_view
    options_from_collection_for_select(available_projects, :id, :name, :selected => selected_projects)
  end
